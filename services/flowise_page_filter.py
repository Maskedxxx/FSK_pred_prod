...
<response_example>
Пример батча [21, 22, 23, 24, 25]:
- Страницы 21-24: методология, общее описание объекта
- Страница 25: "Кухня. Ламинат: вздутие кромок, щели до 3мм. Нарушение СП 29.13330"

Ответ: {{"reason": "На странице 25 начинается детальный перечень дефектов с указанием помещений и нарушений", "start_page": 25, "found": true}}
</response_example>
...
PROMPT_SEARCH_END = """Ты аналитик строительной документации. Твоя задача — найти, где ЗАКАНЧИВАЕТСЯ список дефектов в <pages_to_analyze>.
...
<gaps_in_list>
ВАЖНО ПРО ДЫРКИ (1-2 страницы без дефектов внутри списка <pages_to_analyze>):
...
Конец — когда описания дефектов (помещение + элемент + повреждение + норма) больше НЕ возвращаются и соответсвуют <end_of_list_definition>.
</gaps_in_list>
...
<pages_to_analyze>
ТАК ВЫГЛЯДЯТ СТРАНИЦЫ ДЛЯ АНАЛИЗА:

{pages_content}
</pages_to_analyze>

<response_example>
Пример батча [25, 26, 27, 28, 29, 30]:
- Страницы 25-28: описания дефектов
- Страница 29: начинается "Локальная смета на устранение..."

Ответ: {{"reason": "На странице 29 начинается локальная смета", "end_page": 28, "found": true}}
</response_example>

<task>
Проанализируй страницы выше. Найди:
Найди страницу где НАЧИНАЕТСЯ один из разделов: СМЕТА, ВЫВОДЫ, ПРИЛОЖЕНИЯ, ЗАКЛЮЧЕНИЯ
Если найден такой маркер — верни end_page = номер предыдущей страницы (последняя с дефектами), found = true.
Если маркер не найден в батче — верни end_page = -1, found = false.
</task>
...
def _search_end(
    ...
    for i in range(0, len(pages_from_start), batch_size):
        ...
        end_page = parsed.get("end_page")
        found = parsed.get("found", False)
        reason = parsed.get("reason", "")

        logger.info("  end_page=%s, found=%s", end_page, found)
        logger.info("  reason: %s...", reason[:100] if reason else "")

        # Если найден маркер конца (смета/выводы/приложения)
        if found and end_page is not None and int(end_page) > 0:
            logger.info(">>> НАЙДЕН КОНЕЦ: последняя страница с дефектами = %s", end_page)
            return int(end_page), debug_info

    # Если дошли до конца документа без явного маркера конца —
    # возвращаем последнюю страницу документа как конец списка дефектов
    last_page = pages_from_start[-1].page_number if pages_from_start else start_page
    logger.info("Документ закончился без маркера конца. Последняя страница: %s", last_page)
    return last_page, debug_info
