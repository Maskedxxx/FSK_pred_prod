...
VLM_CLEAN_PROMPT = (
    ...
    "Форматирование:\n"
    "- Не упускай не одного слова при форматировании, весь текст КРИТИЧСЕСКИ ВАЖНО сохранить в полной форме. Даже если он не вписывается в форматирование MD то просто сохрание его как есть на месте где он имеется сверху или снизу на изображения!\n"
    ...
    "Весь текст включая текст включая вне таблицы (если он есть на изображениее но как то не входит в таблицу!), вне загаловков (текст без загаловка обязательно тоже сохраняем) сохраняйте в ответе в отдельном блоке.\n"
